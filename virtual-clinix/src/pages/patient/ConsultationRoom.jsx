...
const PatientConsultationRoom = () => {
  ...
  const fetchAppointmentDetails = async () => {
    const { data } = await supabase
      .from('appointments')
      .select(`*, doctors(full_name, email, qualification)`)
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center">
      <div className="text-center text-white">
        ...
        <p className="text-gray-400">Setting up your consultation room...</p>
      </div>
    </div>
  )
  ...
  if (error) return (
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center">
      <div className="text-center text-white">
        <p className="text-red-400 text-xl mb-4">⚠️ Failed to load consultation room</p>
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-[#08465A] border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-300 text-sm">
            Consultation with Dr. {doctor?.full_name || '...'}
          </span>
        </div>
